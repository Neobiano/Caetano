...
{ // verifica se valor de dns automático ou manual
	...
	for ($contador=1; $contador<49; $contador++)
	{
			...
			//for($pos_dia=01; ( $pos_dia<($qtd_dias+1) ); $pos_dia++) //aqui
		  	for($pos_dia=01; ( $pos_dia<(10+1) ); $pos_dia++)
		  	{			
...
echo "<hr>";
	...
	//for($pos_dia=01; ( $pos_dia<($qtd_dias+1) ); $pos_dia++) //aqui
	{
		...
		//Gerando um array com todas as FILAS		 
        ...
        //---------------------------------------Preenchendo valores de A, B, TMA e SC----------------------------------------------//
        //Valor de A e aplicando na tabela
        ...
        for($q=0; $row = $query->fetch(); $q++)
        {
            ...
            //if (isset($tabela[$pos_dia][$cod_fila]["VALOR_A"]))
            $tabela[$pos_dia][$cod_fila]["VALOR_A"] = $total; 
        ...
        {        
            ...
            if (isset($tabela[$pos_dia][$cod_fila]["VALOR_B"]))            
                $tabela[$pos_dia][$cod_fila]["VALOR_B"] = $total; 
            
            if (isset($tabela[$pos_dia][$cod_fila]["VALOR_TMA"]))            
                $tabela[$pos_dia][$cod_fila]["VALOR_TMA"] = $tma;                                       
        }
        ...
        for($q=0; $row = $query->fetch(); $q++)
        {
            ...
            if (isset($tabela[$pos_dia][$cod_fila]["VALOR_B"]))            
                $tabela[$pos_dia][$cod_fila]["VALOR_C"] = $total;
        }
        ...
        for($q=0; $row = $query->fetch(); $q++)
        {
            ...
            if (isset($tabela[$pos_dia][$cod_fila]["VALOR_SC"]))            
                $tabela[$pos_dia][$cod_fila]["VALOR_SC"] = $total;
        }
        
		//Valor de CA a totalização é por ilhas
		$soma_ansm = 0;
        $cont_ansm = 0;
        $qtde_ca = 0;
		...
		for ($i = 0; $i <= 12; $i++)
        {
            ...
            $ilha = retornaIlhaIndice($i);                    
            
            //Valor CA
            $sql = "SELECT  COD_FILA, COUNT (*) TOTAL   FROM  TB_EVENTOS_DAC ted
                                    inner join  (
                                                    SELECT distinct  CALLID, min(data_hora) d_hora
                                                    FROM TB_EVENTOS_DAC
                                                    WHERE DATA_HORA BETWEEN '$qual_mes/$pos_dia/$qual_ano' AND '$qual_mes/$pos_dia/$qual_ano 23:59:59.999' 
                                                    AND CALLID IS NOT NULL AND TEMPO_ATEND > '0' and COD_FILA in ($ilha) 
                                                   GROUP BY CALLID
                                    ...
                                    AND ted.CALLID IS NOT NULL AND ted.TEMPO_ATEND > '0' AND ted.COD_FILA in ($ilha)
                                    GROUP BY ted.COD_FILA";                                                
                        
                        
            $q_query = $pdo->prepare($sql);
            $q_query->execute();
            
            for($q=0; $q_row = $q_query->fetch(); $q++)
            {
                $cod_fila = $q_row['COD_FILA'];
                $qtde_ca = $q_row['TOTAL'];                                            
                $valor_a = $tabela[$pos_dia][$cod_fila]["VALOR_A"];                               
                $valor_b = $tabela[$pos_dia][$cod_fila]["VALOR_B"];
                $valor_c = $tabela[$pos_dia][$cod_fila]["VALOR_C"];                   
                $valor_sc = $tabela[$pos_dia][$cod_fila]["VALOR_SC"];                                                                   
                ...
                // Verificando se as ligações tal do shortcall
                ...
                if ($qtde_ca > 0)
                {
                    ...
                    $tabela[$pos_dia][$cod_fila]["VALOR_NSA"]= $valor_nsa;                    
                    $tabela[$pos_dia][$cod_fila]["VALOR_NSR"]= $valor_nsr;
                    $tabela[$pos_dia][$cod_fila]["VALOR_NS"]= $valor_nivel_de_servico;                                                                                                
                    $tabela[$pos_dia][$cod_fila]["VALOR_PCT_SC"]= $perc_sc;
                    $tabela[$pos_dia][$cod_fila]["QTDE_CA"]= $qtde_ca;
                    $tabela[$pos_dia][$cod_fila]["VALOR_BRUTO"]= $valor_pg;
                    $tabela[$pos_dia][$cod_fila]["VALOR_PCT_ACP"]= $imp_acp_aplicado;
                    $tabela[$pos_dia][$cod_fila]["QTDE_ACP"]= $qtde_acp;
                    $tabela[$pos_dia][$cod_fila]["AUT_ACP"]=$acp_aut; 
                    $tabela[$pos_dia][$cod_fila]["FATOR_ACP"]=$fator;
                    $tabela[$pos_dia][$cod_fila]["APL_MULT_ANSM"]= 0.00;// a partir daqui dados da 'segunda' tabela, estes serao calculados posteriormente
                    $tabela[$pos_dia][$cod_fila]["AD_ACP"]= 0.00;
                    $tabela[$pos_dia][$cod_fila]["APLI_MULT_ACP"]= 0.00;                                                                                                          
            ...
            } //final FOR - Retorno das consultas da consulta da ilha especifica
                                       		         																																				
		} //final FOR - Retorno do laço de 1 a 12 nas ilhas
		...
		{
			$cod_fila = $vet_todas_filas[$i];        			                			                           										    		    
		    $qtde_ca = $tabela[$pos_dia][$cod_fila]["QTDE_CA"];
                        
            if ($qtde_ca > 0)
            {   
				$valor_pg = $tabela[$pos_dia][$cod_fila]["VALOR_BRUTO"];			
				$qtde_acp = $tabela[$pos_dia][$cod_fila]["QTDE_ACP"];
                $fator	= $tabela[$pos_dia][$cod_fila]["FATOR_ACP"];		
				...
				$tabela[$pos_dia][$cod_fila]["APL_MULT_ANSM"]	= $aplicacao_ansm;
				$tabela[$pos_dia][$cod_fila]["AD_ACP"]	= $ad_acp;
				$tabela[$pos_dia][$cod_fila]["APLI_MULT_ACP"]	= $apli_mult_acp;  
		...
		for ($i = 0; $i < $count; $i++)
		{
		    ...
		    if ($qtde_ca > 0)    			          
			{			    
				$cod_fila = $tabela[$pos_dia][$cod_fila]["COD_FILA"];
				$nome_fila = $tabela[$pos_dia][$cod_fila]["NOME_FILA"];
				$valor_a = $tabela[$pos_dia][$cod_fila]["VALOR_A"];
				$valor_b = $tabela[$pos_dia][$cod_fila]["VALOR_B"];  
				$valor_c = $tabela[$pos_dia][$cod_fila]["VALOR_C"];										
				$valor_nsa = number_format($tabela[$pos_dia][$cod_fila]["VALOR_NSA"], 10, ',', '.');					
				$valor_nsr = number_format($tabela[$pos_dia][$cod_fila]["VALOR_NSR"], 2, ',', '.');					  
				$valor_nivel_de_servico = number_format($tabela[$pos_dia][$cod_fila]["VALOR_NS"], 10, ',', '.');					
				$valor_tma = $tabela[$pos_dia][$cod_fila]["VALOR_TMA"];
				$valor_sc = $tabela[$pos_dia][$cod_fila]["VALOR_SC"];  
				$perc_sc = number_format($tabela[$pos_dia][$cod_fila]["VALOR_PCT_SC"], 2, ',', '.');
				$valor_pg = number_format($tabela[$pos_dia][$cod_fila]["VALOR_BRUTO"], 2, ',', '.'); 
		...
		for ($i = 0; $i < $count; $i++)
		{
			...
			if ($qtde_ca > 0)                              
			{				
				$acp_aut = $tabela[$pos_dia][$cod_fila]["AUT_ACP"];				
				$cod_fila = $tabela[$pos_dia][$cod_fila]["COD_FILA"];
				$nome_fila = $tabela[$pos_dia][$cod_fila]["NOME_FILA"];
				$valor_pg = number_format($tabela[$pos_dia][$cod_fila]["VALOR_BRUTO"], 2, ',', '.');
				$aplicacao_ansm = number_format($tabela[$pos_dia][$cod_fila]["APL_MULT_ANSM"], 2, ',', '.');
				$imp_acp_aplicado = $tabela[$pos_dia][$cod_fila]["VALOR_PCT_ACP"];
				$qtde_acp = $tabela[$pos_dia][$cod_fila]["QTDE_ACP"];
				$ad_acp	= number_format($tabela[$pos_dia][$cod_fila]["AD_ACP"], 2, ',', '.');									
				$apli_mult_acp = number_format($tabela[$pos_dia][$cod_fila]["APLI_MULT_ACP"], 2, ',', '.');	
