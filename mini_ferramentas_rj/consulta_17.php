...
<?php
$nome_relatorio = "pesquisa_de_satisfacao"; // NOME DO RELATÃ“RIO (UTILIZAR UNDERLINE, POIS Ã‰ PARTE DO NOME DO ARQUIVO EXCEL)
...
// IMPRIME COLUNAS DA TABELA - INÃ�CIO
...
for($j=1;$j<=4;$j++){
	$perg = "perg$j";
	$query = $pdo->prepare("select cod_fila vnd, $perg resposta, count(*) total from tb_pesq_satisfacao
							where data_hora between '$data_inicial' and '$data_final 23:59:59.999'
							group by cod_fila, $perg");
	$query->execute(); // EXECUTA A CONSULTA
	
	// IMPRIME O RESULTADO DA CONSULTA - INÃ�CIO
	for($i=0; $row = $query->fetch(); $i++){
		
		// RECEBE RESULTADOS DA CONSULTA - INÃ�CIO
...
//IMPRIME INFORMAÃ‡Ã•ES DA TABELA - INÃ�CIO
...
//IMPRIME INFORMAÃ‡Ã•ES DA TABELA - FIM

include "finaliza_tabela.php"; // FINALIZA A TABELA
//include"imprime_grafico.php"; // IMPRIME O GRÃ�FICO
...
for($j=1;$j<=4;$j++){
	...
	echo "<tr class='w3-indigo'>";
		...
		if($perg == 'perg1') echo "<td><b>No Geral, qual seu grau de satisfação?</b></td>";
		if($perg == 'perg2') echo "<td><b>Quanto ao tempo de espera, você se considera:</b></td>";
		if($perg == 'perg3') echo "<td><b>Quanto à cordialidade do atendente, você se considera:</b></td>";
